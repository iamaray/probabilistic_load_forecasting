...
class BayesTrainer:
    def __init__(self, model_wrapper: BSMDeTWrapper = None, train_loader=None,
                 ...
                 train_norm=None, modelsave=False, device=None):
    ...
    def _save_model(self):
        torch.save(self.net, os.path.join(self.savepath, 'bmdet.pt'))
        np.save(os.path.join(self.savepath, "mu_list"), self.mu_list)
        np.save(os.path.join(self.savepath, "rho_list"), self.rho_list)
        np.save(os.path.join(self.savepath, "loss_train"), self.loss_train)
...
def grid_search_torch_model(model_class: nn.Module,
    ...
    futures = []
    # If you only have one GPU available, max_workers should equal 1.
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        for i, params in enumerate(param_combinations):
            param_dict = dict(zip(param_grid.keys(), params))
            gpu_id = i % max_workers  # make sure this index is valid for your hardware
